...
- `project_id`: 클라이언트가 소속된 프로젝트 ID (선택)
...
|----------|------|------|------|
| page | number | O | 기본 1 |
| limit | number | O | 기본 20 |
| status | string | X | 상태 필터: `pending` \| `in_progress` \| `completed` |
| project_id | string | X | 프로젝트 필터 |
| q | string | X | 제목/내용 검색어 |
...
**Response 204** (No Content) 또는 **Response 200** `{}`
...
| ------------ | ----------------------------------- |
| Method       | `POST`                              |
| Path         | `/admin/api/inquiries/{id}/replies` |
| 인증         | 필수                                |
| Content-Type | `application/json`                  |
